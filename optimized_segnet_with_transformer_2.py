from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, UpSampling2D, Concatenate, Multiply, Add, Dense, LayerNormalization
...
def transformer_block(x, num_heads=2, ff_dim=64):
...
def attention_transformer_block(x, g, filters):
    ...
    return transformer_block(attention_applied, num_heads=2, ff_dim=filters)

def optimized_segnet_with_transformer_2(input_shape):
    ...
    model = Model(inputs, outputs, name="Optimized_SegNet_with_Transformer_2")
    return model
